  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
        <h1 className="text-3xl font-bold text-center mb-6">Quiz-Ergebnisse</h1>
        <h2 className="text-xl text-center text-gray-600 mb-8">{quiz.Thema}</h2>
            ...
            return (
              ...
              >
                <div className="flex items-start mb-2">
                  <span className={`mr-2 text-2xl ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                    {isCorrect ? '✓' : '✗'}
                  </span>
                  <div className="flex-1">
                    ...
                    <div className="ml-4 space-y-1">
                      ...
                      {userAnswer?.selectedAnswers.map(aIndex => (
                        <div key={aIndex} className="ml-4 text-sm">
                      ...
                      {!isCorrect && (
                        <>
                          <p className="text-sm mt-2">
                            <span className="font-medium text-green-700">Richtige Antwort(en):</span>
                          </p>
                          {question.Antworten.filter(a => a.Richtig).map((answer, aIndex) => (
                            <div key={aIndex} className="ml-4 text-sm text-green-700">
                              • {answer.Antwort}
                            </div>
                          ))}
                        </>
